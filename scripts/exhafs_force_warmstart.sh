...
# Ensemble member with ENSID <= ${ENS_FCST_SIZE} will run the full-length NHRS forecast
  ...
  glob_cdmbgwd=${glob_cdmbgwd_ens:-1.0}
...
#cdmbgwd_nml=$( echo ${cdmbgwd} | cut -d , -f 1-${nest_grids} )
#cdmbgwd_nml=${cdmbgwd}
...
for n in $(seq 1 ${nest_grids}); do
  ...
  #cdmbgwd_nml=$( echo ${cdmbgwd} | cut -d , -f 1,${n} )
  #cdmbgwd_nml=${cdmbgwd}
...
# Ghassan.Alaka@noaa.gov, 2024-07-06: Add logic to link the parent domain and appropriate nest/tile files to the real storm COMhafs with the appropriate nest0[N] and tile[N] files linked to files named with nest02 and tile2. This would allow atm_vi to find the correct files
if [ ${RUN_MULTISTORM} == "YES" ] && [ ${RUN_INIT:-NO} = "NO" ]; then
 ...
 for sid in ${multistorm_sids} ; do
  COMhafs_nest=${COMhafs/00L/${sid^^}/}
  out_prefix_nest=${out_prefix/00l/${sid,,}/}
  RESTARTout_nest=${COMhafs_nest}/${out_prefix_nest}.RESTART
  mkdir -p ${RESTARTout_nest}
  cd ${RESTARTout_nest}

  # Link/copy parent domain files to RESTARTout_nest
  find ${RESTARTout} \( ! -name "*nest0[0-9]*" \) -exec ${NCP} -rp {} ${RESTARTout_nest}/ \;

  # Link/copy appropriate nest files to RESTARTout_nest
  find ${RESTARTout} \( -name "*nest0${tileno}*" \) -exec ${NCP} -rp {} ${RESTARTout_nest}/ \;

  # If tileno>2, rename destination files 
  if [ "${tileno}" != "2" ]; then
   rename "nest0${tileno}" "nest02" *nest0${tileno}*
   rename "tile${tileno}" "tile2" *tile${tileno}*
  fi
